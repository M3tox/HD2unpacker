...
#include <fstream>
#include <Windows.h>
// commctrl only needed if you deal with windows GUI
#include "commctrl.h"
...
public:
	...
	// Constructor needs path to file you want to extract
	ISDM(const std::string&);

	// destructor closes file and clears table record from heap
	~ISDM();

	void CloseFile();

	void WriteTableRecords(const char*);
	void WriteFileHeaders(const char*);
	void WriteFileNames(const char*);

	void WriteSingleFile(uint32_t, const std::string&);
	// extract all files to location where HD2unpacker is
	void WriteAllFiles();
	// extract all files to location where HD2unpacker is but specifiy start and get counter
	void WriteAllFiles(uint32_t&);
	// this function was specifically made for the windows API
	// if windows API isnt used, this function can be left out
	void WriteAllFiles(bool&, uint32_t&, HWND, HWND, const std::string&);

	// file name in archive including local path
	const char* GetFileName();
	// get file index by passing Name. Mind this function reads only the 16 byte long HINT
	...
	// use this if you just want to know if there is an error
	uint16_t GetLastError();

	// path where the .dta is read from.
	// used by the browse directory prompt in order to jump user to same folder
	std::string filePath;


	// the fileBuffer is a little helper to manage the allocation of file content
	// it will resize to the max file size ever given and will recycle the allocated content
	// in order to avoid new heap allocations for each file
	// it would be possible to define a single instance of this class for each ISDM object
	// but I decided to do it depending on its functions, so the buffer can be kept alive
	...
	public:
		...
		// used to be a char array, but string is easier to handle
		std::string FileName;
...
private:
	...
	// the names shown here are just for reference and are not being matched
	// those will be shown in the title within the window
	const char* origFileNames[KEYCOUNT]
	{
		{"Maps"},
		{"Maps_C"},
		{"Maps_U"},
	...
	// used for wave file decompression
	const uint16_t WAV_DELTAS[896]
	{
	...
	struct DTA_ArchiveHeader {
		...
		uint32_t   ahFileTableOffset;		// offset to file table record
		uint32_t   ahFileTableSize;			// size of actual file table
		uint32_t   ahExtra;					// unknown var
	};
	...
	// table entry
	struct DTA_FileTableRec {
		uint16_t	checkSum;			// 8bit checksum of upper filename
		uint16_t	fileNameLength;		// filename length
		uint32_t	HeaderOffset;		// offset to file header, or where header starts in file (position)
		uint32_t	DataOffset;			// offset to file dat, or position where entry starts in file (position)
		char		filenameHint[16];	// filename hint
	};

	// additional header for wave files
	struct WavHeader
	{
		uint32_t wChunkId;
		uint32_t wChunkSize;
		...
		uint32_t wChunkId2;
		uint32_t wChunkSize2;
		uint16_t wTags;
		uint16_t wChannels;
		uint32_t wSamplesPerSecond;
		uint32_t wBytesPerSecond;
		uint16_t wBlockAlign;
		uint16_t wSampleBits;

		uint32_t wChunkId3;
		uint32_t wChunkSize3;
	};

	// this struct is only used for ISD1 format in order to read
	// all block information in two steps
	struct DTA_BlockHeaderISD1 {
		DTA_BlockHeaderISD1(uint32_t& BlockCount) {
			BlockSizes = new uint32_t[BlockCount];
			BlockTypes = new uint8_t[BlockCount];
		}
	...
	int32_t errStatus{ 0 };
	bool bISD1;
	int8_t keySelector{ -1 };

	DTA_ArchiveHeader ArchiveHeader;
	WavHeader wavHeader;
	...
	inline void Decrypt(void* pData, uint32_t length);
	static bool WriteToFile(void*, uint32_t, std::string&, std::string);
